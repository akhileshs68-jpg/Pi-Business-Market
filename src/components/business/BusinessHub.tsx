    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
              {[
                ...
                { label: 'Marketing Center', path: '#marketing', icon: Megaphone, action: () => setActiveTab('marketing'), desc: 'Coupons & Ads' },
                { label: 'Verification', path: '#verification', icon: ShieldCheck, action: () => setActiveTab('verification'), desc: 'Regulatory Compliance' },
              ].map((mod, idx) => (
                <button
                  key={idx}
      ...
      {activeTab === 'stores' && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-black text-white">Registered Store Outlets</h3>
          ...
          {overview?.stores && overview.stores.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {overview.stores.map(store => (
                <div key={store.storeId} className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 space-y-4 hover:border-slate-700 transition-all">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-base font-extrabold text-white">{store.storeName}</h4>
                        <p className="text-xs text-slate-500">{store.city || 'Main Outlet'}, {store.country || 'Global'}</p>
                      </div>
                    </div>
                  ...
                  <p className="text-xs text-slate-400 line-clamp-2">{store.description || 'Primary merchant fulfillment center.'}</p>

                  <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs">
                    <span className="text-slate-500 font-medium">Type: {store.storeType || 'Retail Store'}</span>
                    <button 
                      onClick={() => navigate(`/store-dashboard`)} 
                      className="text-indigo-400 hover:text-indigo-300 font-bold flex items-center gap-1"
                    >
                      Manage Outlet <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
              ))}
            ...
            <div className="py-16 text-center bg-slate-900/30 rounded-3xl border border-slate-800">
              <StoreIcon className="w-12 h-12 text-slate-700 mx-auto mb-3" />
              <h4 className="text-base font-bold text-white">No Additional Stores Created</h4>
              <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 mb-6">Create branch outlets to extend your local reach across regions.</p>
              <button 
                onClick={() => navigate('/create-store')} 
                className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-2xl text-xs"
              >
                Create First Outlet
              </button>
            </div>
      ...
      {activeTab === 'catalog' && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 sm:p-8 space-y-6">
              <div className="p-4 bg-indigo-600/10 border border-indigo-500/20 rounded-2xl w-fit">
                <Package className="w-8 h-8 text-indigo-400" />
              </div>
              <div>
                <h3 className="text-xl font-extrabold text-white">Physical & Digital Products</h3>
                <p className="text-xs text-slate-400 mt-1">Manage physical inventory, variants, digital assets, and discounts.</p>
              </div>
              <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                <span className="text-xs font-bold text-slate-500 uppercase">Active Products: {overview?.productsCount || 0}</span>
                <button 
            ...
            <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 sm:p-8 space-y-6">
              <div className="p-4 bg-emerald-600/10 border border-emerald-500/20 rounded-2xl w-fit">
                <Briefcase className="w-8 h-8 text-emerald-400" />
              </div>
              <div>
                <h3 className="text-xl font-extrabold text-white">Professional & Business Services</h3>
                <p className="text-xs text-slate-400 mt-1">Manage consultation packages, service hours, and client appointments.</p>
              </div>
              <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                <span className="text-xs font-bold text-slate-500 uppercase">Active Services: {overview?.servicesCount || 0}</span>
                <button 
                  onClick={() => navigate('/service-management')} 
                  className="px-5 py-2.5 bg-emerald-600 text-white text-xs font-bold rounded-xl hover:bg-emerald-500 transition-all"
                >
                  Open Service Hub
                </button>
              </div>

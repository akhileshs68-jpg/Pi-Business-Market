...
content = re.sub(r'\{\(order\.orderStatus === OrderStatus\.READY_FOR_PICKUP \|\| order\.orderStatus === OrderStatus\.SHIPPED \|\| order\.orderStatus === OrderStatus\.OUT_FOR_DELIVERY\) && \(\s*<>\s*<button.*?</>\s*\)\}', ts_error_fix, content, flags=re.DOTALL)

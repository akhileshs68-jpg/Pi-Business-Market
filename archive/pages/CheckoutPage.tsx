...
export const CheckoutPage: React.FC = () => {
  ...
  const loadCheckoutData = async () => {
    ...
    try {
      ...
      if (allItems.length === 0 && import.meta.env.VITE_DEVELOPMENT_MODE === 'true') {
        allItems = [
          {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 relative overflow-hidden pb-28 sm:pb-28 lg:pb-28">
      ...
      <Navbar 
        ...
        onWalletUpdate={(val) => {
          ...
          localStorage.setItem('pi_wallet_balance', String(val));
        }}
                  ...
                  <div className="pt-2">
                    <PiPaymentButton 
                      ...
                      metadata={{
                        ...
                        uid: user!.uid,
                        businessId: (items.filter(item => selectedItemIds.length === 0 || selectedItemIds.includes(item.itemId))[0] as any)?.businessId || 'PI-CORP-001',
                        storeId: (items.filter(item => selectedItemIds.length === 0 || selectedItemIds.includes(item.itemId))[0] as any)?.storeId || 'PI-STORE-001',
                        amount: orderSummaryData.totalAmount,
                        memo: `Checkout Payment for Order #${orderSummaryData.orderNo}`,
                      }}
                      onSuccess={async (paymentId, txid) => {
                        ...
                        // Deduct wallet balance and save to localStorage
                        const currentBalanceStr = localStorage.getItem('pi_wallet_balance') || '300';
                        const currentBalance = parseFloat(currentBalanceStr);
                        const newBalance = Math.max(0, currentBalance - orderSummaryData.totalAmount);
                        localStorage.setItem('pi_wallet_balance', String(newBalance));
                        setWalletBalance(newBalance);
                        console.log(`[CheckoutPage] Balance updated on payment success. Old: ${currentBalance}, Deducted: ${orderSummaryData.totalAmount}, New: ${newBalance}`);

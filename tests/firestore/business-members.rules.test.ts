...
describe('Firestore Security Rules - 21 Adversarial Scenarios', () => {
  beforeAll(async () => {
    try {
      ...
      console.warn('Firestore emulator not running on 127.0.0.1:8080; skipping rules unit tests in this environment.');
  ...
  test('1. Buyer creates Owner membership -> DENIED', async (ctx) => {
    if (!emulatorAvailable || !testEnv) return ctx.skip();
  ...
  test('2. Seller creates Owner membership in another business -> DENIED', async (ctx) => {
    if (!emulatorAvailable || !testEnv) return ctx.skip();
  ...
  test('3. Business Admin creates Owner membership -> DENIED', async (ctx) => {
    if (!emulatorAvailable || !testEnv) return ctx.skip();
  ...
  test('4. Business Admin creates normal Staff membership -> ALLOWED', async (ctx) => {
    if (!emulatorAvailable || !testEnv) return ctx.skip();
    const db = testEnv.authenticatedContext('biz_admin_uid').firestore();
  ...
  test('5. Business Owner creates second Owner -> DENIED', async (ctx) => {
    if (!emulatorAvailable || !testEnv) return ctx.skip();
    const db = testEnv.authenticatedContext('owner_a_uid').firestore();
  ...
  test('20. Initial business + Owner membership in SAME transaction -> ALLOWED', async (ctx) => {
    if (!emulatorAvailable || !testEnv) return ctx.skip();
  ...
  test('21. Cross-business membership injection -> DENIED', async (ctx) => {
    if (!emulatorAvailable || !testEnv) return ctx.skip();
    const db = testEnv.authenticatedContext('owner_a_uid').firestore();

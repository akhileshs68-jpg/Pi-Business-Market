...
const incompleteRegex = /app\.post\("\/api\/payments\/incomplete", async \(req, res\) => \{[\s\S]*?\}\);/;
...
const match = code.match(incompleteRegex);
if(match) {
  code = code.replace(incompleteRegex, match[0] + "\n  " + newStatusEndpoint);
